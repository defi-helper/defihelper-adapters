...
export function Protocol(props) {
  ...
  React.useEffect(async () => {
    ...
    await adaptersGateway.load(props.protocol);
  ...
  const onWalletReload = async () => {
    ...
    try {
      const [metrics, actions] = await Promise.all([contractMetrics.wallet(wallet), contractMetrics.actions(wallet)]);
  ...
  const onAction = async (method) => {
    ...
    try {
      ...
      const actionResult = await action[method].apply(action, JSON.parse(actionParams));
      setActionResult(actionResult instanceof Error ? actionResult.toString() : actionResult);
    } catch (e) {
      console.error(e);
    }
  ...
  return (
    <div className="container">
      ...
      <div className="row">
        ...
        <div className="column">
          <label>Address: </label>
          <input type="text" placeholder="0x" value={contract} onChange={(e) => setContract(e.target.value)} />
        </div>
        <div className="column">
          <button className="button" onClick={onContractReload} disabled={contractReload}>
      ...
      {!contractMetrics || (
        <div>
          <div>
            <ReactJson src={JSON.parse(JSON.stringify(contractMetrics))} collapsed={1} />
          </div>
          <div className="row">
            <div className="column">
              <label>Wallet:</label>
            ...
            <div className="column">
              <button className="button" onClick={onWalletReload} disabled={walletReload}>
          ...
          {!walletMetrics || (
            <div>
              <div>
                <ReactJson src={JSON.parse(JSON.stringify(walletMetrics))} collapsed={1} />
              </div>
            </div>
          )}
          {!actions || (
            <div>
              <div className="row">
                ...
                <div className="column">
                  <button className="button" onClick={() => onAction('can')} disabled={actionsReload}>
                    Can
                  </button>{' '}
                  <button className="button" onClick={() => onAction('send')} disabled={actionsReload}>
                    Send
                  </button>
                </div>
              </div>
              <div>
                <textarea value={actionParams} onChange={(e) => setActionParams(e.target.value)}></textarea>
              </div>
