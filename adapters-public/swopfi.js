...
var toFloat$1 = {
  toFloat: (n, decimals) => new bn$2(n.toString()).div(new bn$2(10).pow(decimals)),
};
    ...
    if (prev[token]) {
      return {
        ...
        [token]: Object.entries(data).reduce(
          (prev, [k, v]) => ({
            ...prev,
            [k]: prev[k] ? new bn$1(prev[k]).plus(v).toString(10) : v,
          }),
          prev[token]
        ),
      };
...
const { ethers: ethers$1, dayjs, axios: axios$1 } = lib;
...
function bridgeWrapperBuild(aliases, blockTag, block, network) {
  return (address) => {
    const alias = aliases[address.toLowerCase()] ?? aliases[ethers$1.utils.getAddress(address)];
...
const { axios, bn, wavesSigner, wavesSeedProvider, wavesTransaction } = lib;
const { waves } = waves_1;
...
var swopfi = {
  ...
  automates: {
    ...
    autorestake: async (signer, contractAddress) => {
      const netByte = await signer.getNetworkByte();
      waves.nodes[netByte] ?? waves.nodes.main;
