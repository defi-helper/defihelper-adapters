...
const { ethers: ethers$5, bn: bn$6, ethersMulticall: ethersMulticall$2 } = lib;
...
const ethereum$6 = {
  ...
  erc20: (provider, address) => new ethers$5.Contract(address, ERC20ABI, provider),
  ...
  dfh: {
    ...
    storage: (provider, address) => new ethers$5.Contract(address, DFHStorageABI, provider),
    storageKey: (k) => ethers$5.utils.keccak256(ethers$5.utils.toUtf8Bytes(k)),
  },
  uniswap: {
    PairInfo: class {
      constructor({
    ...
    pair: (provider, address) => new ethers$5.Contract(address, UniswapPairABI, provider),
    ...
    router: (provider, address) => new ethers$5.Contract(address, UniswapRouterABI, provider),
...
const { ethers: ethers$4, dayjs, axios } = lib;
...
function bridgeWrapperBuild$1(aliases, blockTag, block, network) {
  return (address) => {
    const alias = aliases[address.toLowerCase()] ?? aliases[ethers$4.utils.getAddress(address)];
...
const { ethereum, tokens } = utils;
const EthStakingABI = require$$2;
const EthAutomateABI = require$$3;
